...
if not _TRAIN_MODEL:
    simple_classifier.load_state_dict(torch.load("classifier_v2_5_to_18_h512_d8_00010.pt"))
    state = _SOLVED_STATE
    task = []
    for _ in range(35):
    ...
    while True:
        ...
        for i, state in enumerate(beam):
            ...
            if pick > beam_size:
                pick = beam_size
            min_ind = torch.topk(scores.flatten(), pick, largest=False).indices
        ...
        pick = beam_scores.size(0)
        if pick > beam_size:
            pick = beam_size
        min_ind = torch.topk(beam_scores.flatten(), pick, largest=False).indices
...
print(low.shape, high.shape, middle[0].shape)
...
for epoch in range(100):
    ...
    if epoch % 1 == 0:
        with torch.no_grad():
            output = simple_classifier(test_input[:10000])
            loss = loss_classifier(output, test_output[:10000])
        ...
        with torch.no_grad():
            ...
            print(epoch, loss.item())
    if epoch % 1 == 0:
        torch.save(simple_classifier.state_dict(), f"classifier_v2_{_LOW_CLASS}_to_{_HIGH_CLASS}_h{_HIDDEN}_d{_CLASSIFIER_DEPTH}_{epoch:05}.pt")
